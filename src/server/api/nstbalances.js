...
let url = require('url');
let nstbalancesRouter = express.Router();
const {
  odaByarg
...
const {
  ...
  odasearchby,
  getallbyid$
} = require('../../server/features/nstbalance/index').toinit();
const {
  getapistreamdata$,
  getapiObserver,
...
nstbalancesRouter.get('/nstbalances', function (req, res) {
  const getdata = index$$;
  return getapistreamdata$(getdata).subscribe(getapinotification(req, res));
});

nstbalancesRouter.get('/nstbalances/:id', function (req, res, next) {
...
nstbalancesRouter.post('/nstbalances', function (req, res, next) {
...
nstbalancesRouter.delete('/nstbalances/:id', function (req, res, next) {
  console.log(req.url + ' : querying for ' + req.params.id);
...
nstbalancesRouter.put('/nstbalances/:id', function (req, res, next) {
  let requestBody = req.body;
  console.log(req.url + ' : querying for ' + req.params.id);
...
module.exports = nstbalancesRouter;
